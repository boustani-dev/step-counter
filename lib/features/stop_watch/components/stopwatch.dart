  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<StopWatchBloc, StopWatchState>(
      ...
      builder: (context, state) {
        double progressValue = state.status.duration.toDouble() / 0.01;
        return buildCircularProgressIndicator(
            size, progressValue, state, context);
      },
    );
  }

  SizedBox buildCircularProgressIndicator(Size size, double progressValue,
      StopWatchState state, BuildContext context) {
    return SizedBox(
        ...
        children: [
          Container(
            ...
            child: CircularProgressIndicator(
              value: state.status.duration.toDouble() / 0.01,
              strokeWidth: 30,
